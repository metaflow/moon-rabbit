...
class TwitchClient(twitchio.Client):
    ...
    async def setup_hook(self) -> None:
        ...
        for channel_name, info in self.channels.items():
            ...
            if not uid:
                logging.warning(
                    f"[setup_hook] could not resolve user ID for channel {channel_name!r}"
                )
                continue
    ...
    async def event_oauth_authorized(self, payload) -> None:
        logging.info(
            f"event_oauth_authorized: access_token={payload.access_token}, refresh_token={payload.refresh_token}"
        )
    ...
    async def event_message(self, payload: twitchio.ChatMessage) -> None:
        try:
            ...
            info: ChannelInfo | None = self.channels.get(channel_name)
            if not info:
            ...
            def get_vars():
                ...
                if not variables:
                    variables = {
                        ...
                        "random_mention": Lazy(
                            lambda: self.random_mention(info, author), stick=False
                        ),
    ...
    async def event_channel_hype_train_end(self, payload) -> None:
        try:
            ...
            contributors: str = ", ".join(
                "@" + c.user.name for c in (payload.top_contributions or [])
            )
            channel_id = info.channel_id
            log = InvocationLog(f"twitch channel {channel_name} ({channel_id})")
    ...
    def random_mention(self, info: ChannelInfo, author: str) -> str:
        users = [x for x in info.active_users if x != author]
