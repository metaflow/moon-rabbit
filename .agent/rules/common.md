...
7. **Exact Dependency Versions**: Always pin dependencies to exact versions (`==`) in `requirements.txt`. Never use ranges (`>=`, `<`, `~=`). This prevents subtle differences between dev and prod environments.
8. Use `uv` for all dependency management and running scripts (e.g. `uv pip install`, `uv run`). Do not use `pipenv` or `Pipfile`.