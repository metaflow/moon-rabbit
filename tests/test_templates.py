...
from data import render, Action, ActionKind
import commands
...
class MockDB:
    ...
    def count_variables_in_category(self, channel_id, category):
        return sum(1 for k in self.variables.keys() if k[0] == channel_id and k[2] == category)

    def delete_category(self, channel_id, category):
        keys_to_delete = [k for k in self.variables.keys() if k[0] == channel_id and k[2] == category]
        for k in keys_to_delete:
            del self.variables[k]

    def list_variables(self, channel_id, category):
        return [(k[1], v) for k, v in self.variables.items() if k[0] == channel_id and k[2] == category]

    def get_random_text_id(self, channel_id, tag_query):
        # simple mock
        if "apple" in tag_query: return 1
        if "banana" in tag_query: return 2
        return None
